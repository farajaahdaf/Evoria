@php
    ...
    $avatarUrl = $user && $user->profile_photo_path
        ...
        : "https://ui-avatars.com/api/?name=" . urlencode($user->name ?? 'Organizer') . "&background=2563EB&color=ffffff&size=128";

    $createEventUrl = optional($user?->organizerProfile)->status === 'verified'
    ...
    <div class="max-w-[1400px] mx-auto px-6 h-[80px] flex items-center justify-between gap-6">
        <div class="flex items-center gap-8">
            ...
            <nav class="hidden lg:flex items-center gap-6">
                ...
                <a class="text-[14px] font-bold text-slate-900 hover:text-primary transition-colors" href="{{ $createEventUrl }}">Buat Event</a>
            </nav>
        </div>
        ...
        <div class="flex-1 max-w-[500px] hidden md:block">
            <div class="relative">
                ...
                <input class="w-full h-[44px] pl-11 pr-4 bg-[#F1F3F5] border-none rounded-lg text-[13px] placeholder:text-slate-400 focus:ring-1 focus:ring-primary focus:bg-white transition-colors" placeholder="Cari event dan venue..." type="text"/>
            </div>
        </div>
        ...
        <div class="relative flex items-center gap-3">
            ...
            >
                <span class="material-symbols-outlined text-[20px] transition-transform duration-200" :class="organizerMenuOpen ? 'rotate-180' : 'rotate-0'">expand_more</span>
            </button>
            ...
            >
                <a href="{{ route('organizer.dashboard') }}" class="block px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-primary transition-colors">Dashboard Organizer</a>
                <a href="{{ route('organizer.events.index') }}" class="block px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-primary transition-colors">Kelola Event</a>
                <a href="{{ $createEventUrl }}" class="block px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-primary transition-colors">Buat Event</a>
