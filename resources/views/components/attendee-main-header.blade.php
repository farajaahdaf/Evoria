    ...
    <div class="max-w-[1400px] mx-auto px-6 h-[80px] flex items-center justify-between gap-6">
        <div class="flex items-center gap-8">
            ...
            <nav class="hidden lg:flex items-center gap-6">
                <a class="text-[14px] font-bold text-slate-900 hover:text-primary transition-colors" href="{{ route('attendee.dashboard') }}">Tiket Saya</a>
            </nav>
        </div>
        ...
        <div class="relative flex items-center gap-3">
            ...
            <div class="hidden sm:block">
                <p class="text-[12px] text-slate-500 leading-none">Attendee</p>
                <p class="text-[14px] font-bold text-slate-900 leading-tight">{{ $user->name }}</p>
            </div>
            <button
                type="button"
                @click="attendeeMenuOpen = !attendeeMenuOpen"
                ...
                aria-label="Buka menu attendee"
            >
                <span
                    class="material-symbols-outlined text-[20px] transition-transform duration-200"
                    :class="attendeeMenuOpen ? 'rotate-180' : 'rotate-0'"
                >expand_more</span>
            </button>

            <div
                x-cloak
                x-show="attendeeMenuOpen"
                @click.away="attendeeMenuOpen = false"
                ...
                class="absolute right-0 top-14 w-44 bg-white border border-slate-200 rounded-xl shadow-lg py-2 z-50"
            >
